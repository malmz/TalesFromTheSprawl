...
class ArtifactsCog(commands.Cog, name="network"):
    ...
    async def old_connect_command(self, ctx, code: str = None, password: str = None):
        ...
        await self.log_connect_attempt(
            str(ctx.message.author.id), code, password, announcement
        )
...
artifacts_conf_file = str(config_dir / artifacts_conf_dir / "artifacts.json")
...
def create_artifact(name: str, content: str = None):
    ...
    i = 2
    while os.path.exists(config_dir / artifacts_conf_dir / filename):
        filename = f"{name}_{i}.data"
        i += 1

    # Write data file
    out_path = config_dir / artifacts_conf_dir / filename
