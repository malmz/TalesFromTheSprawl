...
class GmCog(commands.Cog, name=gm_role_name):
    ...
    async def add_known_handle_command(self, interaction: Interaction, handle_id: str):
        if handle_id is None:
            await interaction.response.send_message(
                "Error: provide a handle", ephemeral=True
            )
        else:
            player_setup.add_known_handle(handle_id)
            await interaction.response.send_message(
                f"Added entry for {handle_id}. Please update its contents manually by editing the file.",
                ephemeral=True,
            )
    ...
    @app_commands.command(
        name="create_scenario", description="GM-only. Create a basic scenario."
    )
    @app_commands.checks.has_role(gm_role_name)
    ...
    @app_commands.command(
        ...
        description="GM-only. Create an artifact.",
        # 		help=(
    ...
    @app_commands.checks.has_role(gm_role_name)
    async def create_artifact_command(
    ...
    @app_commands.command(
        name="init_gm", description="GM-only. Reinitialise the GM context and handles."
    )
    @app_commands.checks.has_role(gm_role_name)
