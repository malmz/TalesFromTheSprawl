...
async def create_chat(ctx):
	...
	creator_current_handle = handles.get_handle(creator_player_id)
	...
	# Chat-specific config: will hold all history, but also players' active handles
	chat_state = ConfigObj(f'{chats_dir}/{chat_file_name}')
	chat_state[creator_player_id] = {}
	chat_state[creator_player_id][handle_index] = creator_current_handle
	chat_state.write()

	channel_name = chat_name
	channel = await players.create_chat_channel(ctx.guild, creator_player_id, channel_name)
	channel_id = channel.id
	clickable_channel_ref = channels.clickable_channel_ref(channel)
	chats[chat_name][channel_id_index] = channel_id
	chats.write()
	
	print(f'{creator_player_id}, {creator_player_id}, {creator_current_handle}, {chat_name}, {chat_file_name}')
