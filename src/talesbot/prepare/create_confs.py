...
def get_json_from_row(row):
    ...
    for handle_data in secondary_handles_money_str.split(","):
        ...
        if name in alt_handles_names:
            ...
            log_error(
                "Found money amount for handle but it is not declared as alt handle, npc handle or burner"
            )
    ...
    print(
        f"INFO: {len(alt_handles_names)+len(burners_names)+len(npc_handles_names)} handles are without money entry"
    )
