...
class ShopExtension(Extension, name="shop"):
    ...
    async def order(
        ...
        report = await order_product_from_command(
            str(ctx.user.id), shop_name, product_name
        )
        if report is None:
...
class EmployeeExtension(Extension, name="employee"):
    ...
    async def create_shop(self, ctx: SlashContext, shop_name: str, player_id: str):
        await ctx.defer(ephemeral=True)
...
async def create_shop(
    shop_name: str, handle_id: str, is_owner: bool = False
) -> ActionResult:
